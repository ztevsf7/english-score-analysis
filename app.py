...
def process_data(files):
    ...
    for file in files:
        ...
        try:
            ...
            # 2. 识别总分列 (修改点：增加了 '科目成绩')
            # 新表头中总分在 "科目成绩" 列
            total_score_col = next((c for c in df.columns if '最新得分' in c or '总分' in c or '科目成绩' in c), None)
            
            # 3. 识别各小题/题型列 (修改点：增加了 '写作', '单词')
            # 你的新表头包含：写作, 语法, 单词, 写作2
            # 这里的关键字只要包含在列名中就会被选中 (例如 '写作' 会匹配 '写作' 和 '写作2')
            keywords = ['听力', '阅读', '五', '完形', '语法', '文', '续写', '填空', '写作', '单词']
    ...
    with tab1:
        ...
        if student_list:
            ...
            if subjects:
                ...
                st.warning("未检测到细分题型列（如写作、语法等）")

            # 3. 统计数据
            st.write("#### 个人均分与稳定性 (标准差越小越稳定)")
            # 仅计算存在的列
            cols_to_stat = ['总分'] + subjects
            p_stats = s_data[cols_to_stat].agg(['mean', 'std']).round(2).T
